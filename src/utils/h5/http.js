...
import { getToken, removeRefreshToken, removeToken } from '@/utils/h5/auth'
import crypto from '@/utils/h5/crypto'
import { serialize, tansParams } from '@/utils/h5/util'
import { isURL, validatenull } from '@/utils/h5/validate'

const baseUrl = import.meta.env.VITE_API_BASE_URL || ''

// 与路由守卫共享：登录路径与回跳地址处理
const isLoginPath = (p = '') => p === '/login' || p.startsWith('/login/')
function getIntendedFullPath(routerInstance) {
...
function resolveIntendedForRedirect(routerInstance) {
  const full = getIntendedFullPath(routerInstance)
  if (!full || full === '/' || isLoginPath(full))
    return '/'
  return full
}
...
function getTokenExpiredMessage() {
  return translate('common.errors.tokenExpired', '用户令牌过期，请重新登录')
}
...
function toastError(message, cfg) {
  ...
  uni.showToast({ title: resolvedMessage, icon: 'error', duration: 20000 })
}
